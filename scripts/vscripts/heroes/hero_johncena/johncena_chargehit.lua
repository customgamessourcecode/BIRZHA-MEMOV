...
LinkLuaModifier( "modifier_JohnCena_Chargehit", "heroes/hero_johncena/modifier_JohnCena_Chargehit", LUA_MODIFIER_MOTION_NONE )
...
function JohnCena_Chargehit:OnSpellStart()
	if IsServer() then
		...
		self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_JohnCena_Chargehit", kv )
	end
	return true
end
...
function JohnCena_Chargehit:GetPlaybackRateOverride()
	return 0.4
end

--------------------------------------------------------------------------------

function JohnCena_Chargehit:GetCastRange( vLocation, hTarget )
	if IsServer() then
		if self:GetCaster():FindModifierByName( "modifier_JohnCena_Chargehit" ) ~= nil then
			return 99999
		end
	end
...
function JohnCena_Chargehit:PlayClawAttackSpeech( bShapeshift )
	if IsServer() then
