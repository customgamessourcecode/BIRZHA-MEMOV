...
function UpdateTimerParticle( event )
	...
	local number = GameRules:GetGameTime() - ability.brew_start - brew_explosion - 0.1
...
function EndBrewing( event )
	...
	caster:RemoveModifierByName("modifier_PrankOnPemp_active")

end	

function EndBrewing2( event )
...
function CheckSelfStun( event )
	...
	if ability.time_charged >= brew_explosion then
		...
		ability:ApplyDataDrivenModifier(caster, caster, "modifier_prankonpemp_stun", {duration = max_stun})
...
function ConcoctionHit( event )
	...
	local mainAbility = caster:FindAbilityByName("BigRussianBoss_PrankOnPemp")
	...
	for _,unit in pairs(heroes_around) do
		...
		ability:ApplyDataDrivenModifier(caster, unit, "modifier_prankonpemp_stun", { duration = stun_duration})
	end
