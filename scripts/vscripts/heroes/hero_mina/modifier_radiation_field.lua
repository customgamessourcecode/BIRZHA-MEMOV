...
function modifier_radiation_field:OnCreated( kv )
...
function modifier_radiation_field:OnRefresh( kv )
	self.radius = self:GetAbility():GetSpecialValueFor( "radius" )
	self.damage_health_pct = self:GetAbility():GetSpecialValueFor( "damage_health_pct" )
end
...
function modifier_radiation_field:OnAbilityExecuted( params )
	if IsServer() then
		...
		local hTalent = self:GetParent():FindAbilityByName( "special_bonus_unique_earth_spirit" )
		if hTalent and hTalent:GetLevel() > 0 then
			flDamagePct = flDamagePct + 2
		end
