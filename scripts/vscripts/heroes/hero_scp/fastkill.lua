function CheckBackstab(params)
	...
	if result_angle >= (180 - (105 / 2)) and result_angle <= (180 + (105 / 2)) then 
		...
		ApplyDamage({victim = params.target, attacker = params.attacker, damage = params.target:GetAttackDamage() * 3, damage_type = ability:GetAbilityDamageType()})
	else
...
function CheckBackstabChance(params)
	...
	if caster:IsIllusion() == false then
		if target:IsAncient() == false then
			if result_angle >= (180 - (105 / 2)) and result_angle <= (180 + (105 / 2)) then 
				ApplyDamage({victim = params.target, attacker = params.attacker, damage = 1000000, damage_type = ability:GetAbilityDamageType()})
