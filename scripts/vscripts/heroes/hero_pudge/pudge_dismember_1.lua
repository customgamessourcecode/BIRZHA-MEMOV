pudge_dismember_1 = class({})
LinkLuaModifier( "modifier_dismember_lua", "heroes/hero_pudge/modifier_dismember_lua.lua" ,LUA_MODIFIER_MOTION_NONE )


function pudge_dismember_1:GetConceptRecipientType()
	return DOTA_SPEECH_USER_ALL
end

function pudge_dismember_1:SpeakTrigger()
	return DOTA_ABILITY_SPEAK_CAST
end

function pudge_dismember_1:GetChannelTime()
...
function pudge_dismember_1:OnAbilityPhaseStart()
	if IsServer() then
...
function pudge_dismember_1:OnSpellStart()
	if self.hVictim == nil then
		return
	end
...
function pudge_dismember_1:OnChannelFinish( bInterrupted )
	if self.hVictim ~= nil then
