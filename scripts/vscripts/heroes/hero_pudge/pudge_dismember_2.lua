pudge_dismember_2 = class({})
LinkLuaModifier( "modifier_dismember_lua", "heroes/hero_pudge/modifier_dismember_lua.lua" ,LUA_MODIFIER_MOTION_NONE )


function pudge_dismember_2:GetConceptRecipientType()
	return DOTA_SPEECH_USER_ALL
end

function pudge_dismember_2:SpeakTrigger()
	return DOTA_ABILITY_SPEAK_CAST
end

function pudge_dismember_2:GetChannelTime()
...
function pudge_dismember_2:OnAbilityPhaseStart()
	if IsServer() then
...
function pudge_dismember_2:OnSpellStart()
	if self.hVictim == nil then
		return
	end
...
function pudge_dismember_2:OnChannelFinish( bInterrupted )
	if self.hVictim ~= nil then
